  ...
  return (
    <article className="bg-gray-900 text-white min-h-screen transition-colors duration-300">
      ...
      <div className="relative w-full h-80 md:h-96">
        <img src={featuredImage} alt={post.title.rendered} className="object-cover w-full h-full" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent" />
        <div className="absolute top-6 left-6">
          <Link href="/blog" className="flex items-center gap-2 text-white hover:text-gray-200 transition bg-black/30 px-4 py-2 rounded-full">
            <ArrowLeft size={18} />
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-2/3">
          <header className="mb-12">
            ...
            {post._embedded?.author?.[0] && (
              <div className="flex items-center gap-3 mt-6">
                <img src={post._embedded.author[0].avatar_urls?.['48']} alt={post._embedded.author[0].name} className="w-10 h-10 rounded-full object-cover" />
                <div>
                  <p className="font-medium">{post._embedded.author[0].name}</p>
                  {post._embedded.author[0].description && (
                    <p className="text-sm text-gray-400">{post._embedded.author[0].description}</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="mt-16 pt-8 border-t border-gray-700">
            <Link href="/blog" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition">
              <ArrowLeft size={18} />
                ...
                <Link key={post.id} href={`/blog/${post.slug}`} className="block group">
                  <div className="flex gap-3">
                    <div className="w-16 h-16 bg-gray-700 rounded overflow-hidden">
                      <img src={post._embedded['wp:featuredmedia']?.[0]?.source_url || ''} alt={post.title.rendered} className="object-cover w-full h-full" />
                    </div>
                    <div>
                      <h4 className="font-medium group-hover:text-blue-400 transition" dangerouslySetInnerHTML={{ __html: post.title.rendered }} />
